...
const Withdraw = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await API.post(`/accounts/${formData.account_id}/withdraw`, {
        amount: parseFloat(formData.amount)
      })

      const amountNum = parseFloat(formData.amount)
      setMessage(`Successfully withdrew ${new Intl.NumberFormat('en-PK', { style: 'currency', currency: 'PKR' }).format(amountNum)}`)
      ...
      setError(err.response?.data?.message || 'Withdrawal failed. Insufficient balance?')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="max-w-4xl mx-auto px-6 py-10">
        ...
        {/* Page Title */}
        <div className="mb-8">
          <h1 className="text-3xl font-light text-gray-900">Withdraw money</h1>
          <p className="text-gray-400 text-sm mt-1">Take money out of your account</p>
        </div>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
          ...
          {/* Amount Input */}
          <div className="p-6 border-b border-gray-50">
            <label className="block text-gray-500 text-sm mb-2">Amount to withdraw</label>
          ...
          {/* Quick Amount Buttons */}
          <div className="p-6 border-b border-gray-50">
            <label className="block text-gray-500 text-sm mb-3">Quick withdraw</label>
            ...
            {selectedAccount && (
              <p className="text-gray-400 text-xs mt-3">
                Max withdrawal: {formatCurrency(selectedAccount.balance)}
              </p>
            )}
          </div>

          {/* Warning for large withdrawals */}
          {selectedAccount && parseFloat(formData.amount) > selectedAccount.balance * 0.5 && parseFloat(formData.amount) > 0 && (
            <div className="mx-6 p-3 bg-amber-50 border border-amber-200 rounded-xl">
              <p className="text-amber-700 text-xs text-center">
                ⚠️ This is a large withdrawal relative to your balance.
              </p>
            </div>
          )}
          ...
          <div className="p-6 bg-gray-50">
            <button
              ...
              disabled={loading || !formData.amount || (selectedAccount && selectedAccount.balance < parseFloat(formData.amount))}
            ...
            >
              {loading ? 'Processing...' : `Withdraw ${formData.amount ? `PKR ${parseFloat(formData.amount).toLocaleString()}` : ''}`}
            </button>
          </div>
        </div>
        ...
        <div className="mt-6 text-center">
          <p className="text-gray-400 text-xs">
            Withdrawals are instant. Daily limit: 200,000 PKR for normal transfers.
          </p>
        </div>
      </main>
    </div>
